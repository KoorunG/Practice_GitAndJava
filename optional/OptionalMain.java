...
public class OptionalMain {
  ...
  public static String getCarInsuranceName(Optional<Person> person) {           //Optional 스트림
  ...
  public Optional<Insurance> nullSafeFindCheapestInsuranceQuiz(Optional<Person> person, Optional<Car> car) {  // 두 Optional을 인자로 받아서
    return person.flatMap(p -> car.map(c -> findCheapestInsurance(p, c)));    // p -> car.map(c -> findCheapestInsurance(p,c))
  }                                                                           // flatMap(p -> c -> (p,c))
